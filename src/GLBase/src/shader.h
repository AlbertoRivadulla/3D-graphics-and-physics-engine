...
struct MaterialWithTextures : Material {
    ...
    unsigned int loadTexture(const char *path, bool sRGB = false, bool hasAlpha = false) {
        ...
        // By default, the image will be flipped vertically, since the (0,0)
        // point in an image is at the upper left, while for OpenGL it is at the
        // lower left. Fix this
        ...
        if (data) {
            ...
            // The 2nd argument is the level of the mipmap on which to generate
            // it. The 3rd argument is the format on which we want to store the
            // texture. The 6th argument must be 0 (legacy). The 7th argument
    ...
    void loadNormalMapTexture(const std::string &path, bool sRGB = false, bool hasAlpha = false) {
        ...
        LOG_WARNING("The logic for a normal map is still not implemented in "
                    "the shader!");
        LOG_WARNING("This would require the computation beforehand of the "
                    "tangent and bitangent vectors of the object");
    }
