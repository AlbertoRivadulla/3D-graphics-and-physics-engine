...
void DeferredRenderer::configureLights(const std::vector<Light *> lights,
    ...
    // It starts in 3, because the three first ones correspond to the three
    // textures of the Geometry pass
...
void DeferredRenderer::endFrame(GLGeometry::GLCubemap *skyMap) {
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, mTargetTexture);
    // Bind the position texture from the G-buffer, to compute the fog
    // contribution
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, mGPositionTexture);
    ...
    // // Recall that the target framebuffer is still bound, so it is used as
    // the
    // // read framebuffer
    // glBlitFramebuffer( 0, 0, mRenderWidth, mRenderHeight,
    //                    0, 0, mWinWidth,    mWinHeight,
