...
void Model::loadModel(std::string path) {
    ...
    //      aiProcess_Triangulate: convert the entire mesh to triangles if they
    //      are not aiProcess_FlipUVs: flip the texture coordinates on the y
    //      axis aiProcess_GetNormals: create normal vectors for each vertex if
    //      the model does not contain them aiProcess_SplitLargeMeshes: split
    //      the mesh into smaller ones aiProcess_OptimizeMeshes: join several
    //      meshes into larger ones, to reduce draw calls
    const aiScene *scene{
...
// Each node contains a set of mesh indices, that point to a particular mesh of
// the scene
void Model::processNode(aiNode *node, const aiScene *scene) {
    ...
    for (int i = 0; i < node->mNumMeshes; ++i) {
        ...
        // Save it in the list of meshes of the model, after doing some
        // processing on it with the member function processMesh.
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial *material,
    ...
    for (int i = 0; i < material->GetTextureCount(type); ++i) {
        ...
        for (int j = 0; j < texturesLoaded.size(); ++j) {
            // Check if the current texture is already in the vector of textures
            // loaded
...
unsigned int loadTextureFromDirectory(const char *path,
...
unsigned int loadTexture(const char *path, bool sRGB, bool hasAlpha) {
    ...
    // By default, the image will be flipped vertically, since the (0,0) point
    // in an image is at the upper left, while for OpenGL it is at the lower
    // left. Fix this
