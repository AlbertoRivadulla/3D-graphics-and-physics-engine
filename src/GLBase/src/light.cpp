...
DirectionalLight::DirectionalLight(glm::vec3 color, glm::vec3 position,
    ...
    // Create a temporary vector, to compute the up direction as being
    // perpendicular to this and the direction vector
...
void DirectionalLight::setupShadowMap() {
    ...
    // Create a uniform buffer object for the light space matrices of each
    // subfrustum
    if (mLightMatricesUBO == 0) // Check if this had already been done before
    {
        ...
        // glGetUniformBlockIndex(mLightingPassShader.ID, "LightSpaceMatrices");
        // glUniformBlockBinding(mLightingPassShader.ID, lights_index, 2);
        // unsigned int lights_index = glGetUniformBlockIndex(mShadowShader->ID,
        // "LightSpaceMatrices"); glUniformBlockBinding(mShadowShader->ID,
        // lights_index, 0); mShadowShader->use();
    }
}
...
void SpotLight::computeShadowMap(
    ...
    // Pass the position of the light and the far plane of its frustum to the
    // shader
