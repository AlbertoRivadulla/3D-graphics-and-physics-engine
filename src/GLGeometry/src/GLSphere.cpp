...
// Constructor
GLSphere::GLSphere(int nrVertices) : mNrVertices{nrVertices + 1} {
    ...
    for (int t = 1; t < mNrVertices; ++t) {
        ...
        for (int p = 0; p < 2 * mNrVertices; ++p) {
            float x{
                0.5f * glm::sin(glm::pi<float>() * (float)t / mNrVertices) *
            ...
            thisVertex.TexCoords =
                glm::vec2(p / (2.f * mNrVertices - 1.f), 0.99 * (y + 0.5f));
    ...
    for (int p = 0; p < 2 * mNrVertices; ++p) {
        ...
        for (int t = 0; t < mNrVertices - 2; ++t) {
            ...
            mIndices.push_back((p + 1) % (2 * mNrVertices) +
                               t * (2 * mNrVertices) + 1);
    ...
    for (int p = 0; p < 2 * mNrVertices; ++p) {
        // Vertical lines
        mIndices.push_back(2 * (mNrVertices * mNrVertices - mNrVertices + 1) -
                           1);
        mIndices.push_back(p + 1 + (mNrVertices - 2) * 2 * mNrVertices);
        mIndices.push_back((p + 1) % (2 * mNrVertices) + 1 +
                           (mNrVertices - 2) * 2 * mNrVertices);
    }
    ...
    //         // Triangles pointing down
    //         mIndices.push_back(p + t * (2 * mNrVertices) + 1);
    //         mIndices.push_back((p + 1) % (2 * mNrVertices) + t * (2 *
    //         mNrVertices) + 1); mIndices.push_back(p + (t + 1) * (2 *
    //         mNrVertices) + 1);
    //
    //         // Triangles pointing up
    //         mIndices.push_back((p + 1) % (2 * mNrVertices) + t * (2 *
    //         mNrVertices) + 1); mIndices.push_back((p + 1) % (2 * mNrVertices)
    //         + (t + 1) * (2 * mNrVertices) + 1); mIndices.push_back(p + (t +
    //         1) * (2 * mNrVertices) + 1);
    //     }
    // }
    // // Join the lower cusp
    ...
    //     // Vertical lines
    //     mIndices.push_back(2 * (mNrVertices * mNrVertices - mNrVertices + 1)
    //     - 1); mIndices.push_back(p + 1 + (mNrVertices - 2) * 2 *
    //     mNrVertices); mIndices.push_back((p + 1) % (2 * mNrVertices) + 1 +
    //     (mNrVertices - 2) * 2 * mNrVertices);
    // }
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                 mIndices.size() * sizeof(unsigned int), &mIndices[0],
                 GL_STATIC_DRAW);
