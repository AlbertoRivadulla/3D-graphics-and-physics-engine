...

{
    ...
    // We'll update the content of the VBO's memory often, so we set it to
    // dynamic
...
void GLGUIRenderer::clearGUI() {
    ...
    // GL_TEXTURE_2D, mGUITextureID, 0);
    // glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
    // mGUITextureID, 0); //Only need to do this once.
    ...
    //
    // // glBindTexture(GL_TEXTURE_2D, mGUITextureID);
    // // glClearTexImage(mGUITextureID, 0, GL_RGBA, GL_UNSIGNED_BYTE, {0, 0, 0,
    // 0});
    // // // glClearTexImage(mGUITextureID, 0, GL_RGBA, GL_UNSIGNED_BYTE,
    // &glm::u8vec4(255, 255, 255, 255)[0]);
    // // // glClearTexImage(TextureName[texture::COLORBUFFER], 0, GL_RGBA,
    // GL_UNSIGNED_BYTE, &glm::u8vec4(255, 255, 255, 255)[0]);
    //
    // glBindTexture( GL_TEXTURE_2D, 0 );
}
