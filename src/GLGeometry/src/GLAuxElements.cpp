...
GLAuxElements::GLAuxElements(int width, int height)
    : mPointShader(
          std::string(BASE_DIR_SHADERS) + "/GLGeometry/pointVertex.glsl",
          std::string(BASE_DIR_SHADERS) + "/GLGeometry/pointFragment.glsl",
          std::string(BASE_DIR_SHADERS) + "/GLGeometry/pointGeometry.glsl"),
      mLineShader(std::string(BASE_DIR_SHADERS) + "/GLGeometry/lineVertex.glsl",
                  std::string(BASE_DIR_SHADERS) +
                      "/GLGeometry/lineFragment.glsl") {
...
void GLAuxElements::setupPoint() {
    ...
    glBufferData(GL_ARRAY_BUFFER, 3 * sizeof(float), &vertex[0],
                 GL_STATIC_DRAW);

    // Set the vertex attribute pointers
    // Vertex position
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 1, GL_FLOAT, GL_FALSE, 3 * sizeof(float),
                          (void *)0);
...
void GLAuxElements::setupLine() {
    ...
    float vertices[]{0.f, 0.f, 0.f, 1.f, 1.f, 1.f};
...
// Method to draw a line
void GLAuxElements::drawRectangle(const glm::vec3 &translation,
...
// Method to draw a parallelepiped with the origin as one of its vertex, and the
// rest spanned by the two vector v1 and v2. If these are not orthogonal, it
// will simply draw a parallelepiped
void GLAuxElements::drawParallelepiped(const glm::vec3 &origin,
...
// Method to draw a line
void GLAuxElements::drawBox(const glm::vec3 &translation,
                            const float &rotationAngle,
                            const glm::vec3 &rotationAxis,
                            const glm::vec3 &scale, const glm::mat4 &view,
                            const glm::mat4 &projection) {
    ...
    for (int y = 0; y <= 1; ++y) {
        for (int angle = 0; angle < mNrVerticesCylinder; ++angle) {
            vertices.push_back(0.5 *
                               glm::cos(2. * glm::pi<float>() * (float)angle /
                                        mNrVerticesCylinder));
            vertices.push_back((float)y - 0.5);
            vertices.push_back(0.5 *
                               glm::sin(2. * glm::pi<float>() * (float)angle /
                                        mNrVerticesCylinder));
        }
    }
...
// Method to draw a line
void GLAuxElements::drawCylinder(const glm::vec3 &translation,
                                 const float &rotationAngle,
                                 const glm::vec3 &rotationAxis,
                                 const glm::vec3 &scale, const glm::mat4 &view,
                                 const glm::mat4 &projection) {
...
void GLAuxElements::setupSphere() {
    ...
    std::vector<float> vertices;
    std::vector<int> indices;
    vertices.reserve(3 * (2 * (mNrVerticesSphere * mNrVerticesSphere -
                               mNrVerticesSphere + 1)));
    ...
    for (int t = 1; t < mNrVerticesSphere; ++t) {
        ...
        for (int p = 0; p < 2 * mNrVerticesSphere; ++p) {
            vertices.push_back(
                0.5 *
                glm::sin(glm::pi<float>() * (float)t / mNrVerticesSphere) *
                glm::cos(glm::pi<float>() * (float)p / mNrVerticesSphere));
            vertices.push_back(0.5 * glm::cos(glm::pi<float>() * (float)t /
                                              mNrVerticesSphere));
            vertices.push_back(
                0.5 *
                glm::sin(glm::pi<float>() * (float)t / mNrVerticesSphere) *
    ...
    for (int p = 0; p < 2 * mNrVerticesSphere; ++p) {
        ...
        for (int t = 1; t < mNrVerticesSphere - 1; ++t) {
            ...
            indices.push_back((p + 1) % (2 * mNrVerticesSphere) +
                              t * (2 * mNrVerticesSphere) + 1);
    ...
    for (int p = 0; p < 2 * mNrVerticesSphere; ++p) {
        ...
        indices.push_back(p + 1 +
                          (mNrVerticesSphere - 2) * 2 * mNrVerticesSphere);
        indices.push_back(2 * (mNrVerticesSphere * mNrVerticesSphere -
                               mNrVerticesSphere + 1) -
                          1);
    }
...
void GLAuxElements::drawSphere(const glm::vec3 &translation,
                               const float &rotationAngle,
                               const glm::vec3 &rotationAxis,
                               const glm::vec3 &scale, const glm::mat4 &view,
                               const glm::mat4 &projection) {
    ...
    glDrawElements(GL_LINES,
                   2 * 2 * mNrVerticesSphere * (2 * mNrVerticesSphere - 1),
                   GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
}
...
void GLAuxElements::setupCone() {
    ...
    for (int angle = 0; angle < mNrVerticesCone; ++angle) {
        vertices.push_back(0.5 * glm::cos(2. * glm::pi<float>() * (float)angle /
                                          mNrVerticesCone));
        vertices.push_back(-0.5);
        vertices.push_back(0.5 * glm::sin(2. * glm::pi<float>() * (float)angle /
                                          mNrVerticesCone));
    }
    // Vertex of the tip
...
// Method to draw a line
void GLAuxElements::drawCone(const glm::vec3 &translation,
                             const float &rotationAngle,
                             const glm::vec3 &rotationAxis,
                             const glm::vec3 &scale, const glm::mat4 &view,
                             const glm::mat4 &projection) {
