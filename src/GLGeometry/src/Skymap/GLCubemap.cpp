...
void GLCubemap::setupNoTextures(const std::string &vertexShaderPath,
                                const std::string &fragmentShaderPath) {
    mHasTexture = false;
    mShader = Shader(vertexShaderPath, fragmentShaderPath);
...
void GLCubemap::loadCubemap(const std::string &texturesPath) {
    ...
    std::vector<std::string> sides{"right",  "left",  "top",
                                   "bottom", "front", "back"};
    // Path of each side
    ...
    for (int i = 0; i < sidesPaths.size(); ++i) {
        ...
        data =
            stbi_load(sidesPaths[i].c_str(), &width, &height, &nrChannels, 0);
