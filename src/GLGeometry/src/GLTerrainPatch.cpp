...
GLTerrainPatch::GLTerrainPatch(unsigned char *data, int width, int height,
    ...
    for (unsigned int i = 0; i < height; i++) {
        for (unsigned int j = 0; j < width; j++) {
            ...
            Vertex thisVertex;
            thisVertex.Position = glm::vec3(
                -0.5f * height + i, (int)y * 0.25f + 16, -0.5f * width + j);
            thisVertex.Normal = glm::vec3(0.f, 1.f, 0.f);
            thisVertex.TexCoords =
                glm::vec2((float)i / width, (float)j / height);

            // Compute the normal, only for the vertices outside the edges of
            // the grid
            if (i > 0 && i < height - 1 && j > 0 && j < width - 1) {

                thisVertex.Normal[0] =
                    0.5f * (data[(j - 1 + width * i) * nChannels] -
                            data[(j + 1 + width * i) * nChannels]);
                thisVertex.Normal[1] = 1.f;
                thisVertex.Normal[2] =
                    0.5f * (data[(j + width * (i - 1)) * nChannels] -
                            data[(j + width * (i + 1)) * nChannels]);
...
void GLTerrainPatch::draw() {
    ...
    for (unsigned int strip = 0; strip < mNumStrips; ++strip) {
        glDrawElements(GL_TRIANGLE_STRIP, // primitive type
                       ...
                       (void *)(sizeof(unsigned int) * mNumVertsPerStrip *
                                strip)); // offset of the starting index
    }
...
GLTerrainTessellated::GLTerrainTessellated(int width, int height)
    ...
    for (unsigned i = 0; i < mNrPatches; i++) {
        for (unsigned j = 0; j < mNrPatches; j++) {
            ...
            vertices.push_back(0.0f);                          // v.y
            vertices.push_back(-height / 2.0f +
                               height * (j + 1) / (float)mNrPatches); // v.z
            vertices.push_back(i / (float)mNrPatches);                // u
