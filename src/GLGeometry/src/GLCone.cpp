...
// Constructor
GLCone::GLCone(int nrVerticesCircle) : mNrVertices{nrVerticesCircle + 1} {
    ...
    mVertices.reserve(
        2 * mNrVertices +
        2); // +2 because of the vertices in the center of the circles
    ...
    for (int angle = 0; angle < mNrVertices; ++angle) {
        ...
        float x{0.5f * glm::cos(2.f * glm::pi<float>() * (float)angle /
                                (mNrVertices - 1))};
        float z{0.5f * glm::sin(2.f * glm::pi<float>() * (float)angle /
                                (mNrVertices - 1))};
