...
void Terrain::addPatchFromTexture(const std::string &heightmapTexPath, float hScale, float vScale, float yShift) {
...
void Terrain::addPatchPlaneTessellated(float hScale, float vScale, float yShift) {
    ...
    int width = 2;
    int height = 2;
    mDataHeight.reserve(4);
    for (int i = 0; i < 4; ++i)
        mDataHeight[i] = 0.f;

    // Write the data for the height map into the texture
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R16F, width, height, 0, GL_RED, GL_FLOAT, mDataHeight.data());
    ...
    computeNormalmapData(width, height, hScale, vScale);
...
// Add a terrain patch from an image
void Terrain::addPatchFromTextureTessellated(const std::string &heightmapTexPath, float hScale, float vScale,
                                             float yShift) {
    // Load the texture for the height map
    int width, height, nChannels;
    unsigned char *data = stbi_load(heightmapTexPath.c_str(), &width, &height, &nChannels, 0);
    if (data) {
        ...
        mDataHeight.reserve(width * height);
        ...
        // GL_UNSIGNED_BYTE, mDataHeight);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, width, height, 0, GL_RED, GL_FLOAT, mDataHeight.data());
        glGenerateMipmap(GL_TEXTURE_2D);
...
void Terrain::addMaterial(std::unique_ptr<GLBase::Material> material) {
...
Shader &Terrain::getTessellationShader() { return mTessellationShader; }

// Compute the normal map given an array of data for the height map
void Terrain::computeNormalmapData(int width, int height, float hScale, float vScale) {
    mDataNormal.reserve(width * height * 4);
    ...
    for (int i = 2; i < width - 2; ++i) {
        for (int j = 2; j < height - 2; ++j) {
            // normal[0] = 0.5f * vScale/hScale * ( (float)data[ (j*width + i-1)
            // ] - (float)data[ (j*width + i+1) ] ); normal[1] = 1.f; normal[2]
            // = 0.5f * vScale/hScale * ( (float)data[ ((j-1)*width + i) ] -
            // (float)data[ ((j+1)*width + i) ] ); normal =
            // glm::normalize(normal);

            normal[0] = 1 / 12.f * vScale / hScale *
                        (-mDataHeight[(j * width + i - 2)] + 8.f * mDataHeight[(j * width + i - 1)] -
                         8.f * mDataHeight[(j * width + i + 1)] + mDataHeight[(j * width + i + 2)]);
            normal[1] = 1.f;
            normal[2] = 1 / 12.f * vScale / hScale *
                        (-mDataHeight[((j - 2) * width + i)] + 8.f * mDataHeight[((j - 1) * width + i)] -
                         8.f * mDataHeight[((j + 1) * width + i)] + mDataHeight[((j + 2) * width + i)]);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, width, height, 0, GL_RGBA, GL_FLOAT, mDataNormal.data());
    glGenerateMipmap(GL_TEXTURE_2D);
}
} // namespace Physics
