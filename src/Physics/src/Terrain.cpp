    ...
    : mTessellationShader(
          Shader(std::string(BASE_DIR_SHADERS) +
                 ...
                 std::string(BASE_DIR_SHADERS) +
                     "/GLGeometry/tessellationGPassFragment.glsl",
                 "",
                 std::string(BASE_DIR_SHADERS) +
                     "/GLGeometry/tessellationGPassTessCtrl.glsl",
                 std::string(BASE_DIR_SHADERS) +
                     "/GLGeometry/tessellationGPassTessEval.glsl")) {}
...
void Terrain::addPatchFromTexture(const std::string &heightmapTexPath,
    ...
    unsigned char *data =
        stbi_load(heightmapTexPath.c_str(), &width, &height, &nChannels, 0);
    // Generate the mesh
    mTerrainPatch = new GLTerrainPatch(data, width, height, nChannels);
    stbi_image_free(data);

    // Add a model matrix to the GLTerrainPatch
    mTerrainPatch->setModelMatrix({0., yShift, 0.}, 0., {0., 0., 1.},
                                  {hScale, vScale, hScale});
...
void Terrain::addPatchPlaneTessellated(float hScale, float vScale,
                                       float yShift) {
    glGenTextures(1, &mHeightmapTex);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D,
                  mHeightmapTex); // all upcoming GL_TEXTURE_2D operations now
...
// Add a terrain patch from an image
void Terrain::addPatchFromTextureTessellated(
    const std::string &heightmapTexPath, float hScale, float vScale,
    float yShift) {
    glGenTextures(1, &mHeightmapTex);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D,
...
void Terrain::addPatchFromHeightDataTessellated(float *heightMapData, int width,
                                                int height, float hScale,
                                                float vScale, float yShift) {
    glGenTextures(1, &mHeightmapTex);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D,
                  mHeightmapTex); // all upcoming GL_TEXTURE_2D operations now
...
void Terrain::computeNormalmapData(float *data, int width, int height,
    ...
    for (int i = 2; i < width - 2; ++i) {
        for (int j = 2; j < height - 2; ++j) {
            ...
            normal[0] =
                1 / 12.f * vScale / hScale *
                (-data[(j * width + i - 2)] + 8.f * data[(j * width + i - 1)] -
                 8.f * data[(j * width + i + 1)] + data[(j * width + i + 2)]);
            normal[1] = 1.f;
