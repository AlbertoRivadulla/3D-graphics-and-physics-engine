...
void PlaneCollider::moveCollider(const glm::mat4 &modelMatrix) {
    // Update the AABB
    computeAABBTransformed(mAABB.verticesModel, modelMatrix,
                           mAABB.cornersWorld);
    ...
    mNormal =
        glm::vec3(glm::normalize(modelMatrix * glm::vec4(0.f, 0.f, 1.f, 0.f)));

    // Compute the two tangent vectors
    mTangent.push_back(
        glm::normalize(glm::vec3(modelMatrix * glm::vec4(1.f, 0.f, 0.f, 0.f))));
    mTangent.push_back(
        glm::normalize(glm::vec3(modelMatrix * glm::vec4(0.f, 1.f, 0.f, 0.f))));
